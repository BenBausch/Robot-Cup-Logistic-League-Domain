...
(:predicates
	...
	(robot5-precedes ?r2 - robot)

   (:private 

  	       ; robots
	       (robot5-at ?l - location)
	       (robot5-at-init)
	       (robot5-holding-material)
	       (robot5-holding-product ?p - product)
	       (robot5-holding-something)
	       (robot5-recently-moved)
	       (robot5-assigned-machine ?m - machine)
   )
...
(:action r5-insert-cap
	:parameters (?m - cap_station ?i - cs_input)
	:precondition (and
		(robot5-assigned-machine ?m)
		(robot5-at ?i)
		(not (conveyor-full ?m))
		(not (robot5-holding-something))
		(input-location ?i ?m)
		(not (cap-buffered ?m))
	)
	:effect (and
		(conveyor-full ?m)
		(material-at ?i)
		(not (robot5-recently-moved))
		(increase (total-cost) 30)
	)
)

(:action r5-pickup-material
...
(:action r5-move-in
	...
	:precondition (and
		(robot5-at-init)
		;(not (exists (?_r - robot) (and (not (robot5-precedes ?_r)) (robot5-at-init))))
