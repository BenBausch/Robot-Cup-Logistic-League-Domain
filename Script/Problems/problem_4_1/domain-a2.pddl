...
(:predicates
	...
	(robot2-precedes ?r2 - robot)

   (:private 

  	       ; robots
	       (robot2-at ?l - location)
	       (robot2-at-init)
	       (robot2-holding-material)
	       (robot2-holding-product ?p - product)
	       (robot2-holding-something)
	       (robot2-recently-moved)
	       (robot2-assigned-machine ?m - machine)
   )
...
(:action r2-insert-cap
	:parameters (?m - cap_station ?i - cs_input)
	:precondition (and
		(robot2-assigned-machine ?m)
		(robot2-at ?i)
		(not (conveyor-full ?m))
		(not (robot2-holding-something))
		(input-location ?i ?m)
		(not (cap-buffered ?m))
	)
	:effect (and
		(conveyor-full ?m)
		(material-at ?i)
		(not (robot2-recently-moved))
		(increase (total-cost) 30)
	)
)

(:action r2-pickup-material
...
(:action r2-move-in
	...
	:precondition (and
		(robot2-at-init)
		;(not (exists (?_r - robot) (and (not (robot2-precedes ?_r)) (robot2-at-init))))
