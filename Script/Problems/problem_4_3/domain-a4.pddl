...
(:predicates
	...
	(robot4-precedes ?r2 - robot)

   (:private 

  	       ; robots
	       (robot4-at ?l - location)
	       (robot4-at-init)
	       (robot4-holding-material)
	       (robot4-holding-product ?p - product)
	       (robot4-holding-something)
	       (robot4-recently-moved)
	       (robot4-assigned-machine ?m - machine)
   )
...
(:action r4-insert-cap
	:parameters (?m - cap_station ?i - cs_input)
	:precondition (and
		(robot4-assigned-machine ?m)
		(robot4-at ?i)
		(not (conveyor-full ?m))
		(not (robot4-holding-something))
		(input-location ?i ?m)
		(not (cap-buffered ?m))
	)
	:effect (and
		(conveyor-full ?m)
		(material-at ?i)
		(not (robot4-recently-moved))
		(increase (total-cost) 30)
	)
)

(:action r4-pickup-material
...
(:action r4-move-in
	...
	:precondition (and
		(robot4-at-init)
		;(not (exists (?_r - robot) (and (not (robot4-precedes ?_r)) (robot4-at-init))))
